...
![img.png](./images/mss.png)

--- 

### TCP 세그먼트 구조  

![img.png](./images/TCP-Segment.png)
...
![img.png](./images/3-way-handshake.png)

![img.png](./images/3-way-handshake-1.png)
...
![img.png](./images/tcp-connection-end.png)
...
![img.png](./images/connection-standby.png)
...
![img.png](./images/connection-establish.png)
...
![img.png](./images/connection-end.png)
...
- 느린 시작 임계치 
  - 혼잡 윈도우 값 증가 중 문제 발생 경우와 해결법
    - 혼잡 윈도우 크기가 느린 시작 임계치 이상이 됨 -> 느린 시작 종료, 혼잡 회피 수행
    - 타임아웃이 발생 -> 혼잡 윈도우 값을 1로, 느린 시작 임계치를 혼잡이 감지되었을 시점의 혼잡 윈도우 값의 절반으로 초기화한 뒤 느린 시작 재개 
    - 세 번의 중복된 ACK 세그먼트를 수신 -> (빠른 재전송 후) 빠른 회복 수행

**혼잡 회피 알고리즘**
- RTT마다 혼잡 윈도우를 1MSS 씩 증가시키는 알고리즘 
- 혼잡 윈도우 크기 선형적으로 증가 
- 느린 시작 임계치를 넘어서면 혼잡 여지가 있으니 조심해서 혼잡 윈도우 증가하기

**빠른 회복 알고리즘**  
- 세 번의 중복 세그먼트 수신 : 빠른 재전송 + 빠른 회복 알고리즘 
...
- 호스트가 ECN을 지원할 경우: TCP/IP 헤더에 ECN 관련 비트 추가
  - IP 데이터그램 헤더의 서비스 유형 필드에 있는 2비트에서 사용
  - 2비트 값이 11인 경우 혼잡 발생 알림
